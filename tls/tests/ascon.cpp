...
START_TEST(encrypt_decrypt_ascon)
{
...
START_TEST(encrypt_decrypt_ad_ascon)
{
  incrementing_nonce nonce(nonce_data);
  ++nonce;
  const auto n = nonce.nonce();
...
START_TEST(encrypt_decrypt_distinct_ascon)
{
  incrementing_nonce nonce(nonce_data);
  ++nonce;
...
START_TEST(encrypt_decrypt_fail_ascon)
{
  incrementing_nonce nonce(nonce_data);
  ++nonce;
...
int main(int argc, char** argv)
{
  ...
  tcase_add_test(tcase, encrypt_decrypt_ascon);
  tcase_add_test(tcase, encrypt_decrypt_ad_ascon);
  tcase_add_test(tcase, encrypt_decrypt_distinct_ascon);
  tcase_add_test(tcase, encrypt_decrypt_fail_ascon);
  suite_add_tcase(suite, tcase);

  tcase = tcase_create("Test vector");
  tcase_add_test(tcase, encrypt_ascon);
  suite_add_tcase(suite, tcase);
